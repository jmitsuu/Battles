...
<template>
  ...
  >
    ...
    <div
      class="w-80 h-52 border-[0.1rem] px-4"
      v-for="item in store.champs"
      :key="store.id"
    >
      <h1 class="text-center font-bold text-white mb-4">{{ item.name }}</h1>
      <div class="absolute ">
        <ul class="list-none font-semibold text-xs text-gray-200">
    ...
    <div class="absolute bottom-2 flex flex-col gap-2">
      <h1 class="text-gray-200">
        Vencedor é: <span class="text-green-500 font-bold">{{ winner }}</span>
      </h1>
      <button
        class="bg-green-800 w-24 mx-auto text-gray-300 py-2 rounded-md hover:bg-green-500"
        @click="batle"
      >
        Vencedor
      </button>
